  ...
  <div class="detail">
    ...
    >
      <el-form-item label="区域表" prop="region">
        <el-select
          v-if="!this.ConflictAnalysisLayerObj"
        ...
        <el-input
          v-else
          :disabled="true"
          v-model="this.ConflictAnalysisLayerObj.layerName"
        ></el-input>
      </el-form-item>
...
export default {
  computed: {
    ...mapState(["ConflictAnalysisLayerObj"]),
  },
  data() {
    return {
  ...
  async created() {
    if (this.ConflictAnalysisLayerObj) {
        console.log(this.ConflictAnalysisLayerObj)
      const res = await this.$api.getTableInformationByTableName(
        this.ConflictAnalysisLayerObj.layerid
      );  
      console.log(res);
  ...
  methods: {
    async getData() {
      ...
      const res = await this.$api.selectStatisticalAnalysisLayers(
        this.ConflictAnalysisLayerObj.layerName
      );
